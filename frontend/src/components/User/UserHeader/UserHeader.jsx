...
function UserHeader() {
    ...
    useEffect (() => {
        ...
        async function checkFollowing() {
            try {
                const res = await $api.get(`/follow/is-following/${targetUserId}`)
    ...
    return (
        <div className="user-header">
            ...
            { username !== authUsername && (
                isFollowing ? (
                    <button className="user-header-follow" onClick={handleUnfollow}>Отписаться</button>
                ) : (
                    <button className="user-header-follow" onClick={handleFollow}>Подписаться</button>
                )
            )}
        </div>
    )
